  ...
  modules.async.waterfall([
    ...
    function(next) {
      modules.request({url: `${url}/pets/${species}`, method: 'GET'}, function(err, response, body) {
        if (err) {
          ...
          const $ = modules.cheerio.load(body);
          const pagerLast = $('.pager-last');
          const index = pagerLast[0].attribs.href.search('page=');
          next(null, pagerLast[0].attribs.href.substr(index + 5));
        }
      });
    },
        ...
        setTimeout(function() {
          ...
          modules.request({url: `${url}${contentURL[i]}`, method: 'GET'}, function(err, response, body) {
            ...
            if (err) {
              ...
              const $ = modules.cheerio.load(body);
              // 寵物資訊
              const link = contentURL[i].replace('/content/', '');
              if (species == 'cat') {
                kind = '貓';
              } else if (species == 'dog') {
                kind = '狗';
              }
              const petName = $('.field-field-pet-name').children().children().text().replace('動物小名:', '').trim();
              let age = $('.field-field-pet-age').children().children().text().replace('動物的出生日（年齡）:', '').replace('了', '').trim();
              age = ageTable(age);
              let neuter = $('.field-field-pet-medical').children().children().text().replace('結紮情況:', '').trim();
              neuter = neuterTable(neuter);
              const countyString = $('.field-field-county').children().children().text();
              const county = countyTable(countyString);
              ...
              const description = $('.field-field-pet-look').children().children().text().replace('簡單描述:', '').trim();
              const habit = $('.field-field-pet-habitate').children().children().text().replace('動物個性略述:', '').trim();
              // 認養資訊
              // var limitation = $('.field-field-pet-limitation').children().children().text().replace('認養資訊:', '').trim();
              const story = $('.field-field-limitation-desc').children().children().text().slice(0, -1);
              const limitation = $('.field-field-pet-limitation').children().children().text().replace('認養條件:', '').split('認養條件:');
              for (let j = 0; j < limitation.length; j++) {
                limitation[j] = limitation[j].trim();
              }
              // 聯絡人資訊
              const contactName = $('.field-field-contact').children().children().text();
              const contactMethod = $('.field-field-tel').children().children().text().substring(255, -1);
              const packData = {link, kind, petName, age, neuter, county, title, image, description, habit, story, limitation, contactName, contactMethod};
    ...
    function(data, next) {
      ...
      for (let i = 0; i < data.length; i++) {
        ...
        loaded++;
        if (loaded === data.length) next(null, insertPet);
      }
    },
    function(petData, next) {
      let loaded = 0;
          ...
          if (err) modules.errorInsert(err, 217);
            ...
            connection.query(`SELECT db_link FROM pet WHERE db_link=${element.db_link} AND db=2`, function(err, result) {
              if (err) modules.errorInsert(err, 219);
                ...
                loaded++;
                if (result.length === 0) { // result.length === 0 'if', should insert data
              ...
              if (loaded === petData.length) console.log(`Adoption map in ${species} update finish.`);
